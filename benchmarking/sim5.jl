...
# Tilo Schwalger
# difference to version 4:
# - transport step for x and z update shifted up front like in sim4.py
# - mu -> mu[i], Delta_u -> Delta_u[i]
# - external stimulus is passed as time series vector Iext[Nsteps_rec, M] (more precisely, a matrix because there is a time series of length Nsteps_rec for each population) within the params dictionary
# - removal of unused seed_stim argument in sim() function
...
function sim(T, dt, dt_rec, params, Nrecord, seed)
	...
	t = dt*ti
	    ...
	    h[i] += dtau[i]*(mu[i] + extInput[i] - h[i]) + synInput[i] * dt
            ...
            n[i, L[i]] = rand(rng, distrib) / N[i] # population activity (fraction of neurons spiking)
