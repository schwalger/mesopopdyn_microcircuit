//changes to version glm_pop_0.9.c:
//corrected threshold update, especially initialization at k=l-K
...
//      A[j]= pop[j].n[(p[j].end-1-ndelay+p[j].K)%p[j].K] / (p[j].N * DT);
      A[j]= pop[j].n[(p[j].end-ndelay+p[j].K)%p[j].K] / DT;
    }
...
void update(struct PopVariablesGLM *pop, double *nmean, double input,gsl_rng *rng,struct PopParametersGLM *p, int mode)
{
  ...
  for (r=0;r < p->N_theta; r++)
    {
      pop->g[r] = pop->g[r]*p->E_theta2[r] + (1 - p->E_theta2[r]) * pop->n[end]/DT;
      theta += p->E_theta[r] * pop->g[r];
    }

  double Pfree, lambda_free_end;
  pop->h = input + (pop->h - p->mu) * p->Em;
  lambda_free_end = cond_rate(pop->h - theta, p);
  Pfree=-expm1(-0.5*(pop->lambda_free+lambda_free_end)*DT);
  pop->lambda_free=lambda_free_end;

  for (k=0; k<K; k++) X += pop->m0[k];

  double ztot,Plam,PLAM,lambda_end, mm;
  ...
  switch(mode)
      ...
      for (k=end;k<=end-kref+K;k++)
	{
	  ...
	  lambda_end = cond_rate(pop->h - threshold, p);//GLM
	  Plam=0.5*(pop->lambda[i]+lambda_end)*DT;
	  if (Plam>0.01) Plam=-expm1(-Plam); 
	  pop->lambda[i]=lambda_end;
	  Z += pop->v[i];
	  mm = Plam * pop->m0[i];
	  Y += Plam * pop->v[i];
	  W += mm;
      ...
      for (k=end;k<=end-kref+K;k++)
	{
	  ...
	  lambda_end = cond_rate(pop->u[i] - threshold, p);//GLIF
	  Plam=0.5*(pop->lambda[i]+lambda_end)*DT;
	  if (Plam>0.01) Plam=-expm1(-Plam);
	  pop->lambda[i]=lambda_end;
  ...
  *nmean = W + Pfree * pop->x + PLAM * (1 - X - pop->x);
  if (p->N <= 1000000)
    pop->n[end] = (double)(gsl_ran_binomial(rng, *nmean, p->N)) /p->N;
  else //pop->n[end]=*nmean;
    if (*nmean>0)
      pop->n[end] = *nmean +  gsl_ran_gaussian_ziggurat(rng, sqrt(*nmean/p->N));
    else
      pop->n[end] = 0;
      ...
      for (i=0;i<Npop;i++)
	{
      ...
      if (l>=L)
      	  ...
      	  for (i=0;i<Npop;i++)
	    {
	      ...
	      //	      if (i==0) printf("k=%d rate=%g nmeanbin=%g\n",k,rate[i][k],nmean_bin[i]);
	      //	      aa[i][k]=a[i];
      	      n_bin[i]=0;
	      nmean_bin[i]=0;
	    }
...
//memory allocation and initialization of variables, 
{
  ...
  for (i=0;i<Npop;i++)
      ...
      for (k=0;k<K;k++)
	{
	  ...
	  p[i].gamma_QR[k] = p[i].deltaV * (1 - exp(-pop[i].theta[k] / p[i].deltaV));
      ...
      /* pop[i].n[0]=1.;   //all neurons are non-refractory (this initialization seems faulty!!!)  */
      /* pop[i].m0[0]=1.; // it gave strange results in Potjans model (does not reproduce Fig.8A of PLOSCB paper)*/
      pop[i].n[K-1]=1.;   //all neurons just spiked and are thus refractory
      pop[i].m0[K-1]=1.;  // these initial conditions reproduce Fig.8A in paper
      /* if (mode>=GLIF) pop[i].u[k]=p[i].mu; */
      pop[i].z=0.;
      pop[i].x=0.;
      //pop[i].x=(p[i].N - n) * DT;
	...
	pop[i].g[r] = pop[i].n[0]/DT/p[i].tau_theta[r];
...
void init_population(struct PopParametersGLM p[], int Npop,double tref[], double taum[], double taus1[][Npop], double taus2[][Npop], double taur1[][Npop], double taur2[][Npop], double a1[][Npop], double a2[][Npop], double mu[], double c[], double deltaV[], double delay[], double vth[], double vreset[], int N[], double J[][Npop], double p_conn[][Npop], double **signal, int N_theta[], double J_ref[], double *J_theta[], double *tau_theta[], double sigma[])
{
  ...
  for (k=0;k<Npop;k++)
      ...
      for (l=0;l<Npop;l++)
	{
	  ...
	  //printf("k=%d l=%d a1=%g a2=%g w1=%g J=%g p=%g taum=%g\n",k,l,a1[k][l],a2[k][l],p[k].w1[l],J[k][l],p_conn[k][l],taum[k]);
	}
    }
