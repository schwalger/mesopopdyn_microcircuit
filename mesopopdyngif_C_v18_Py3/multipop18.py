# change to multipop16.py: based on multipop_c_18.py
# based on glm_popdyn_intensiv_no_lokuptbl.c, which is without lookup table; thus, should be similar to multipop15.py, glm_popdyn_1.0.c but as intensiv version
# does not use the parameter dictionary of multipop17.py
#
...
import multipop_c_18 as mpc #custom-made C simulation of networks and populations
...
#                'V_reset': self.V_th[i]-self.V_reset[i],
            ...
            for j, nest_j in enumerate( self.nest_pops ):
                nest.SetDefaults('static_synapse', {
                    'weight': self.J_syn[i,j] * \
                        self.J_syn_nestfactor[i,j] * self.pconn[i,j], #/ float(self.N[j])
        ...
        self.nest_sd = []
        for i, nest_i in enumerate( self.nest_pops ):
          self.nest_sd.append( nest.Create('spike_recorder') )
          nest.SetStatus( self.nest_sd[i], {'time_in_steps': True})
          nest.SetDefaults('static_synapse', {
             'weight': 1.,
             'delay': self.dt*1e3} )
        ...
        if os.path.exists(fname) and not ForceSim:
            ...
            if self.__sim_mode__==None:
                ...
                if step is not None:
                    ...
                    if not 'nest_stepcurrent' in self.__dict__:
